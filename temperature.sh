...
#%V Make a new logfiles on every new week number
LOG_FILE=$LOG_DIR/`date +%Y_%V`templog.csv

#Make the log dir if it doesn't exist
mkdir -p $LOG_DIR

#Check if the log file exists, if not make it and write the CSV column headers
if [ ! -e "$LOG_FILE" ] ; then
  touch "$LOG_FILE"
...
tempread_G=`cat /sys/bus/w1/devices/28-0316805ff0ff/w1_slave`
tempread_B=`cat /sys/bus/w1/devices/28-0316806035ff/w1_slave`
tempread_W=`cat /sys/bus/w1/devices/28-0516802e56ff/w1_slave`

#Get just the temperature portion (after the =)
temp=${tempread##*=}
temp_R=${tempread_R##*=}
temp_G=${tempread_G##*=}
temp_B=${tempread_B##*=}
